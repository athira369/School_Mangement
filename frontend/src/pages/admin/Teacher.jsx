...
import {
  ...
  AddTeacherInput,
  AddTeacherButton,
} from '../../styles/TeacherStyles'; // Import styled components from TeachersStyles.js
...
const Teachers = () => {
  ...
  const handleAddTeacher = async (e) => {
    ...
    if (newTeacher.name.trim() !== '' && newTeacher.email.trim() !== '' && newTeacher.subject.trim() !== '') {
      try {
        const response = await axios.post('http://localhost:5000/api/v1/teachers', newTeacher);
