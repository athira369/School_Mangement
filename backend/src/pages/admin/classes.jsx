...
const Classes = () => {
  ...
  const fetchClasses = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/v1/class/getall');
  ...
  const handleAddClass = async (e) => {
    ...
    if (newClassName.trim() !== '') {
      try {
        const response = await axios.post('http://localhost:5000/api/v1/class', { grade: newClassName });
